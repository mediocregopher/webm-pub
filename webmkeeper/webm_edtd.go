...
var webmEdtd *edtd.Edtd
func init() {
	webmEdtdBuf := bytes.NewBuffer(webmEdtdB)

	var err error
	webmEdtd, err = edtd.NewEdtd(webmEdtdBuf)
	if err != nil {
		panic(err)
	}
}

// We just say fuck it and put this here. Cause fuck it
var webmEdtdB = []byte(`
// webm is a subset of the matroska spec, so this should be good enough
declare header {
  DocType := "matroska";
  EBMLVersion := 1;
}
      ...
      TrackEntry := ae container [ card:*; ] {
        ...
        Audio := e1 container {
          ...
          OutputSamplingFrequency := 78b5 float [ range:>0.0;
                                                  def:8000.0; ]
          Channels := 9f uint [ range:1..; def:1; ]
